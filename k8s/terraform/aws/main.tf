...
resource "kubernetes_config_map" "grafana_dashboard" {
  ...
  data = {
    "auth-service-dashboard.json" = file("${path.module}/../../../grafana/provisioning/dashboards/auth-service-dashboard.json")
  }
}
    ...
    yamlencode({
      prometheus = {
        additionalPrometheusRulesMap = {
          "auth-service-alerts" = yamldecode(file("${path.module}/../../../prometheus/alerts.yml"))
        }
      }
    })
  ]
